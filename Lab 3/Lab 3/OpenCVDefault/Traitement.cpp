...
void CTraitement::CreateDiagramPercent(Mat frame)
{
	...
	line(Diagram, Point(140, yAxis), Point(140, yAxis - 10), Scalar(0), 2,1);
	line(Diagram, Point(240, yAxis), Point(240, yAxis - 10), Scalar(0), 2,1);
	line(Diagram, Point(340, yAxis), Point(340, yAxis - 10), Scalar(0), 2,1);
	line(Diagram, Point(440, yAxis), Point(440, yAxis - 10), Scalar(0), 2,1);
	line(Diagram, Point(540, yAxis), Point(540, yAxis - 10), Scalar(0), 2,1);

	putText(Diagram, "0",   Point(xAxis,yAxis + 15), CV_FONT_HERSHEY_COMPLEX, 0.5,  Scalar(0,0,200));
	putText(Diagram, "50",  Point(140, yAxis  + 15), CV_FONT_HERSHEY_COMPLEX, 0.5,  Scalar(0,0,200));
	putText(Diagram, "100", Point(240, yAxis  + 15), CV_FONT_HERSHEY_COMPLEX, 0.5,  Scalar(0,0,200));
	putText(Diagram, "150", Point(340, yAxis  + 15), CV_FONT_HERSHEY_COMPLEX, 0.5,  Scalar(0,0,200));
	putText(Diagram, "200", Point(440, yAxis  + 15), CV_FONT_HERSHEY_COMPLEX, 0.5,  Scalar(0,0,200));
	putText(Diagram, "250", Point(540, yAxis  + 15), CV_FONT_HERSHEY_COMPLEX, 0.5,  Scalar(0,0,200));
	...
	for(int i=0; i<frame.cols;i++)
	{
		...
		{
			pixValue = frame.data[j *frame.cols + i]; //0-255
			OccurenceTab[pixValue]++;
		}
	}

	// (parcourir occurence Tab)
	for(int i=0; i< 255;i++)
	{
		yPixel = yAxis - (ResizeY * OccurenceTab[i]);
		xPixel = xAxis + (((double)i/255))*500.0;
	...
	for(int i =0; i < 255; i++)
	{
		if(pow((double)OccurenceTab[indexMaxS1 - i],2) > Max)
		{
			Max = OccurenceTab[i];
			indexMaxS2 = i;
		}
	...
	line(Diagram, Point(((double)indexMaxS2/255) * 500.0, yAxis), Point(((double)indexMaxS2/255) * 500.0, yAxis - 455), Scalar(0), 1,1);
	line(Diagram, Point(((double)indexMaxS1/255) * 500.0, yAxis), Point(((double)indexMaxS1/255) * 500.0, yAxis - 455), Scalar(0), 1,1);
...
void CTraitement::CreateDiagram(int rownumber, Mat frame)
{
	...
	for(int i=0; i<frame.cols;i++)
	{
		pixValue = frame.data[rownumber *frame.cols + i]; //0-255
		...
		xPixel = xAxis + (((double)i/255))*580.0;
